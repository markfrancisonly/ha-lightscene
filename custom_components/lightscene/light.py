...
from collections.abc import Iterable, MutableMapping, Callable, Coroutine
import math
import logging
from typing import Any, Optional
from homeassistant.components.group import ATTR_ENTITIES
import voluptuous as vol


'''
# potential todo: allow for scene brightness scaling and color shift
...
class LightSceneManager:
    ...
    async def async_start_discovery(self, _now: Any) -> None:
        ...
        _LOGGER.debug(
            "Scene discovery started"
        )
        
        for ls in self.lightscenes.values():
            _LOGGER.debug("Removed %s", ls.name)
            await ls.async_remove(force_remove=True)
        ...
        for state in all_states:
            ...
            ls = LightScene(state.name, icon, state.entity_id, scene_entities)
            added_entities.append(ls)

            self.lightscenes[state.entity_id] = ls
    ...
    async def async_scene_reloaded_event_listener(self, event: Event) -> None:
        ...
        _LOGGER.debug(
            "Light scenes manager detected scenes reload"
        )

        # wait for scenes to be reloaded and then repeat discovery
        self._remove_call_later = self.hass.helpers.event.async_call_later(5, self.async_start_discovery)
    ...
    async def async_scene_activated_event_listener(self, event: Event) -> None:
        ...
        try:
            ...
            _LOGGER.error("TypeError: service data %s, entity id is '%s', lightscenes is '%s'", service_data, entity_id, self.lightscenes)
...
class LightScene(LightEntity):
    ...
    def __init__(self, name, icon, scene_entity_id, scene_entity_ids: list[str], entity_filter: vol.Schema = None):
        ...
        self._listener_call_service: Callable[[], None] | None = None
    ...
    async def async_added_to_hass(self):
        ...
        async def async_state_changed_listener(event: Event):
            ...
            if not self._entity_filter is None and not self._entity_filter(entity_id):
                return
            ...
            # todo: consider determining magnitude of change before flipping light scene off
